...
export const createBookingAcceptedNotification = (booking) =>
  createNotification({
    ...
    title: "Người đồng hành đã nhận lịch",
    message: "Người đồng hành đã chấp nhận lịch chăm sóc của bạn. Bạn có thể theo dõi tại trang chi tiết lịch.",
    link: bookingLink(booking),
    bookingId: getBookingId(booking),
...
export const createCompanionCheckedInNotification = (booking) =>
  createNotification({
    ...
    title: "Người đồng hành đã đến nơi",
    message: "Người đồng hành đã đến điểm đón và bắt đầu ca chăm sóc.",
    link: bookingLink(booking),
    bookingId: getBookingId(booking),
...
export const createShiftNoteUpdatedNotification = (booking, shiftLog) => {
  ...
  return createNotification({
    ...
    title: "Người đồng hành vừa cập nhật ghi chú",
    message: note ? `Ghi chú mới: ${note}` : "Người đồng hành vừa cập nhật ghi chú trong ca chăm sóc.",
    link: bookingLink(booking),
    bookingId,
    metadata: { note },
...
export const createBookingCompletedNotification = (booking) =>
  createNotification({
    ...
    message: "Người đồng hành đã kết thúc ca. Báo cáo sau ca đã sẵn sàng để bạn kiểm tra.",
    link: bookingLink(booking),
    bookingId: getBookingId(booking),
    metadata: {
      status: "completed",
...
export const createPaymentReminderNotification = (booking) =>
  createNotification({
    ...
    title: "Nhắc thanh toán lịch chăm sóc",
    message: "Ca chăm sóc đã hoàn thành. Vui lòng thanh toán để hoàn tất lịch và mở phần đánh giá người đồng hành.",
    link: bookingLink(booking),
    bookingId: getBookingId(booking),
    metadata: {
...
export const createCompanionPaymentSuccessNotification = ({ booking, payment }) => {
  ...
  return createNotification({
    ...
    message: `Lịch chăm sóc đã được thanh toán thành công. Thu nhập ca của bạn: ${formatMoney(companionEarning)}.`,
    link: companionBookingLink(booking),
    bookingId: getBookingId(booking),
    metadata: {
      paymentId: toIdString(payment?._id),
...
export const createReviewReminderNotification = (booking) =>
  createNotification({
    ...
    title: "Đánh giá người đồng hành",
